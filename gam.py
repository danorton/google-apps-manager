...
def callGAPIpages(service, function, items, nextPageToken=u'nextPageToken', page_message=None, message_attribute=None, **kwargs):
  pageToken = None
  all_pages = list()
  total_items = 0
  while True:
...
def showReport():
  ...
  if report in [u'users', u'user']:
    ...
    doc_activities = callGAPIpages(service=rep.activities(), function=u'list', items=u'items', page_message=page_message, applicationName=u'docs', userKey=userKey, customerId=customerId, actorIpAddress=actorIpAddress, startTime=startTime, endTime=endTime, eventName=eventName, filters=filters)
    ...
    admin_activity = callGAPIpages(service=rep.activities(), function=u'list', items=u'items', applicationName=u'admin', userKey=userKey, customerId=customerId, actorIpAddress=actorIpAddress, startTime=startTime, endTime=endTime, eventName=eventName, filters=filters)
    admin_attr = []
    ...
    login_activity = callGAPIpages(service=rep.activities(), function=u'list', items=u'items', page_message=page_message, applicationName=u'login', userKey=userKey, customerId=customerId, actorIpAddress=actorIpAddress, startTime=startTime, endTime=endTime, eventName=eventName, filters=filters)
    login_attr = []
    titles = [u'time', u'user', u'event', u'ip']
...
def showDriveFiles(users):
  ...
  for user in users:
    ...
    feed = callGAPIpages(service=drive.files(), function=u'list', items=u'items', page_message=page_message, q=query, maxResults=1000, fields=fields)
    for file in feed:
      a_file = {u'Owner': user}
...
def doDriveSearch(drive, query=None):
  ...
  files = callGAPIpages(service=drive.files(), function=u'list', items=u'items', page_message=page_message, q=query, fields=u'nextPageToken,items(id)', maxResults=1000)
  ids = list()
  for file in files:
...
def showDriveFileTree(users):
  ...
  for user in users:
    ...
    page_message = u' got %%%%total_items%%%% files for %s...\n' % user
    feed = callGAPIpages(service=drive.files(), function=u'list', items=u'items', page_message=page_message, maxResults=1000, fields=u'items(id,title,parents(id),mimeType),nextPageToken')
...
def deleteEmptyDriveFolders(users):
  ...
  for user in users:
    ...
    while deleted_empty:
      ...
      feed = callGAPIpages(service=drive.files(), function=u'list', items=u'items', page_message=page_message, q=query, maxResults=1000, fields=u'items(title,id),nextPageToken')
      deleted_empty = False
      for folder in feed:
...
def transferSecCals(users):
  ...
  for user in users:
    ...
    source_calendars = callGAPIpages(service=source_cal.calendarList(), function=u'list', minAccessRole=u'owner', items=u'items', showHidden=True, fields=u'items(id),nextPageToken')
...
def transferDriveFiles(users):
  ...
  for user in users:
    ...
    page_message = u'  got %%total_items%% files\n'
    source_drive_files = callGAPIpages(service=source_drive.files(), function=u'list', items=u'items', page_message=page_message, q=u"'me' in owners and trashed = false", fields=u'items(id,parents,mimeType),nextPageToken')
    all_source_file_ids = []
    ...
    target_folders = callGAPIpages(service=target_drive.files(), function=u'list', items=u'items', page_message=page_message, q=u"'me' in owners and mimeType = 'application/vnd.google-apps.folder'", fields=u'items(id,title),nextPageToken')
...
def getProductAndSKU(sku):
  product = None
  if sku == u'apps':
    product = u'Google-Apps'
    sku = u'Google-Apps-For-Business'
  elif sku == u'apps-free':
    product = sku = u'Google-Apps'
  elif sku == u'coordinate':
    sku = product = u'Google-Coordinate'
  elif sku == u'vault':
    product = u'Google-Vault'
    sku = u'Google-Vault'
  elif sku in [u'drive-20gb', u'drive20gb', u'20gb']:
    product = u'Google-Drive-storage'
    sku = u'Google-Drive-storage-20GB'
  elif sku in [u'drive-50gb', u'drive50gb', u'50gb']:
    product = u'Google-Drive-storage'
    sku = u'Google-Drive-storage-50GB'
  elif sku in [u'drive-200gb', u'drive200gb', u'200gb']:
    product = u'Google-Drive-storage'
    sku = u'Google-Drive-storage-200GB'
  elif sku in [u'drive-400gb', u'drive400gb', u'400gb']:
    product = u'Google-Drive-storage'
    sku = u'Google-Drive-storage-400GB'
  elif sku in [u'drive-1tb', u'drive1tb', u'1tb']:
    product = u'Google-Drive-storage'
    sku = u'Google-Drive-storage-1TB'
  elif sku in [u'drive-2tb', u'drive2tb', u'2tb']:
    product = u'Google-Drive-storage'
    sku = u'Google-Drive-storage-2TB'
  elif sku in [u'drive-4tb', u'drive4tb', u'4tb']:
    product = u'Google-Drive-storage'
    sku = u'Google-Drive-storage-4TB'
  elif sku in [u'drive-4tb', u'drive8tb', u'8tb']:
    product = u'Google-Drive-storage'
    sku = u'Google-Drive-storage-8TB'
  elif sku in [u'drive-16tb', u'drive16tb', u'16tb']:
    product = u'Google-Drive-storage'
...
def doLicense(users, operation):
  ...
  for user in users:
    ...
    if operation == u'delete':
      ...
      callGAPI(service=lic.licenseAssignments(), function=operation, productId=productId, skuId=skuId, userId=user, body={u'skuId': skuId})
...
def doUpdateNotification():
  ...
  if isUnread == None:
    ...
    sys.exit(3)
  if get_all:
    notifications = callGAPIpages(service=cd.notifications(), function=u'list', items=u'items', customer=customerId, fields=u'items(notificationId,isUnread),nextPageToken')
...
def doDeleteNotification():
  ...
  while i < len(sys.argv):
    if sys.argv[i].lower() == u'id':
      ...
      sys.exit(3)
  if get_all:
    notifications = callGAPIpages(service=cd.notifications(), function='list', items='items', customer=customerId, fields=u'items(notificationId),nextPageToken')
    for noti in notifications:
      ids.append(noti[u'notificationId'])
...
def doGetNotifications():
  ...
  while i < len(sys.argv):
    if sys.argv[i].lower() == u'unreadonly':
      ...
      sys.exit(3)
    i += 1
  notifications = callGAPIpages(service=cd.notifications(), function=u'list', items=u'items', customer=customerId)
...
def doGetOrgInfo():
  ...
  if get_users:
    if name != u'/':
      name = u'/%s' % name
    print u'Users: '
    users = callGAPIpages(service=cd.users(), function=u'list', items=u'users', customer=customerId, query=u"orgUnitPath='%s'" % name, fields=u'users(primaryEmail,orgUnitPath)')
    for user in users:
...
def doPrintLicenses():
  lic = buildGAPIObject(u'licensing')
  products = [u'Google-Apps', u'Google-Drive-storage', u'Google-Coordinate', u'Google-Vault', u'VAULT']
  lic_attributes = [{}]
  todrive = False
  i = 3
  while i < len(sys.argv):
  ...
  for productId in products:
    ...
    try:
      licenses = callGAPIpages(service=lic.licenseAssignments(), function=u'listForProduct', items=u'items', throw_reasons=[u'invalid'], page_message=page_message, customerId=domain, productId=productId, maxResults=1000)
    except apiclient.errors.HttpError:
      licenses = []
